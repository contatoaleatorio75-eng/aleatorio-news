...
const generateImage = async (keywords: string[]): Promise<string> => {
    ...
    if (!ai) {
        console.error("GoogleGenAI client not initialized. Falling back to Unsplash.");
        return fallbackImageUrl;
    }
    try {
        const imagePrompt = `A high-quality, photorealistic image for a news article thumbnail, representing the concept of: "${keywords.join(', ')}". Avoid text and logos.`;
        ...
        console.error(`AI image generation failed for keywords: "${keywords.join(', ')}". Falling back to Unsplash.`, error);
        return fallbackImageUrl;
    }
};
